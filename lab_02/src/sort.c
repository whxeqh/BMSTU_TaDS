...
static void uni_swap(void *left, void *right, const size_t el_size)
{
    ...
    memcpy(right, left, el_size);
    memcpy(right, tmp, el_size);
}

int uni_cmp(const void *left, const void *right, const size_t el_size)
{
    const char *l = left;
    const char *r = right;
    ...
    for (size_t i = 0; i < len; ++i)
        for (size_t j = i + 1; j < len; ++j)
            if (uni_cmp(&countries[i].capital, &countries[j].capital, sizeof(countries[i].capital) > 0))
                uni_swap(&countries[i].capital, &countries[j].capital, sizeof(countries[i].capital));
}
    ...
    for (size_t i = 0; i < len; ++i)
        for (size_t j = i + 1; j < len; ++j)
            if (uni_cmp(keys[i].capital, &keys[j].capital, sizeof(keys[i].capital) > 0))
                uni_swap(&keys[i].capital, &keys[j].capital, sizeof(keys[i].capital));
}
